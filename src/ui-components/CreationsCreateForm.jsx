...
export default function CreationsCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const [shared, setShared] = React.useState(initialValues.shared);
  const [likes, setLikes] = React.useState(initialValues.likes);
  const autocompleteLength = 10;
  const [errors, setErrors] = React.useState({});
  const resetStateValues = () => {
    setPrompt(initialValues.prompt);
    setCostprompt(initialValues.costprompt);
    setGender(initialValues.gender);
    setJewellrytype(initialValues.jewellrytype);
    setMaterial(initialValues.material);
    setKwt(initialValues.kwt);
    setStone(initialValues.stone);
    setGrade(initialValues.grade);
    setGemsource(initialValues.Gemsource);
    setRingsize(initialValues.ringsize);
    setStyle(initialValues.Style);
    setRingstyle(initialValues.ringstyle);
    setGemsize(initialValues.gemsize);
    setCut(initialValues.cut);
    setUrl(initialValues.url);
    setColour(initialValues.colour);
    setShape(initialValues.shape);
    setAccepteddate(initialValues.accepteddate);
    setIpaddress(initialValues.ipaddress);
    setOutstandingbalance(initialValues.outstandingbalance);
    setRequiredby(initialValues.requiredby);
    setEstimatecostfrom(initialValues.Estimatecostfrom);
    setEstimatecostto(initialValues.Estimatecostto);
    setLastupdate(initialValues.lastupdate);
    setStatus(initialValues.status);
    setSubmittedforquote(initialValues.submittedforquote);
    setSubmittedfororder(initialValues.submittedfororder);
    setTermsaccepted(initialValues.termsaccepted);
    setAmontquoted(initialValues.amontquoted);
    setSubmitteddate(initialValues.submitteddate);
    setClarity(initialValues.clarity);
    setBandwidth(initialValues.bandwidth);
    setPrompt_Strength(initialValues.Prompt_Strength);
    setSimilarity_Strength(initialValues.Similarity_Strength);
    setSeedurl(initialValues.seedurl);
    setNecklace_Length(initialValues.Necklace_Length);
    setBracelet_Length(initialValues.Bracelet_Length);
    setSecondary_Gemstone(initialValues.Secondary_Gemstone);
    setEarring_Fastening_Types(initialValues.Earring_Fastening_Types);
    setBracelet_Clasp_Types(initialValues.Bracelet_Clasp_Types);
    setRing_Band_Style(initialValues.Ring_Band_Style);
    setRing_Setting(initialValues.Ring_Setting);
    setNecklace_Chain_Style(initialValues.Necklace_Chain_Style);
    setBracelet_Chain_Style(initialValues.Bracelet_Chain_Style);
    setBrooch_Fastening_Types(initialValues.Brooch_Fastening_Types);
    setGem_Settings(initialValues.Gem_Settings);
    setNecklace_Clasp(initialValues.Necklace_Clasp);
    setClientID(initialValues.clientID);
    setCurrentClientIDValue(undefined);
    setCurrentClientIDDisplayValue("");
    setShared(initialValues.shared);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createCreations.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "CreationsCreateForm")}
      {...rest}
    >
      <TextField
        label="Prompt"
      ...
      <SwitchField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.shared ?? value;
          }
          if (errors.shared?.hasError) {
            runValidationTasks("shared", value);
          }
          setShared(value);
        }}
        onBlur={() => runValidationTasks("shared", shared)}
        errorMessage={errors.shared?.errorMessage}
        hasError={errors.shared?.hasError}
