...
import { getArtisan, getContact, listArtisans } from "../graphql/queries";
...
export default function ContactUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = contactRecord
      ? { ...initialValues, ...contactRecord, artisanID }
      : initialValues;
    setArtisanID(cleanValues.artisanID);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : contactModelProp;
      const artisanIDRecord = record ? record.artisanID : undefined;
      const artisanRecord = artisanIDRecord
        ? (
            await client.graphql({
              query: getArtisan.replaceAll("__typename", ""),
              variables: { id: artisanIDRecord },
            })
          )?.data?.getArtisan
        : undefined;
      setArtisanID(artisanIDRecord);
      setSelectedArtisanIDRecords([artisanRecord]);
      setContactRecord(record);
    };
    queryData();
  }, [idProp, contactModelProp]);
  React.useEffect(resetStateValues, [contactRecord, artisanID]);
