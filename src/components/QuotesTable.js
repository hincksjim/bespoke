...
export default function QuotesTable() {
  ...
  const navigateToQuoteDetails = (designId) => {
    navigate(`/Portal/quote-details/${designId}`);
  };

  return (
    <main role="main">
      <h1>Submitted Designs for Quotes</h1>
      <div>
        <input
          type="text"
          ...
          {currentDesigns.map((design) => (
            <div key={design.id} className="design-card">
              ...
              <div className="design-info">
                ...
                <button onClick={() => navigateToQuoteDetails(design.id)} className="btn">
                  Go to Quote Details
                </button>
              </div>
            </div>
